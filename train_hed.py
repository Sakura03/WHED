...
from functions import WassersteinLoss, w_loss_lp
...
parser.add_argument('--tmp', help='tmp folder', default='tmp/WHED')
# ================ dataset
...
# Compute Ground Metric Matrix
ones = torch.ones(100, 100).cuda()
idx = ones.nonzero().double()

tmp = torch.einsum('ij,ij->i', (idx, idx))
XX = tmp.unsqueeze(1)
YY = tmp.unsqueeze(0)
distances = torch.einsum('ik,jk->ij', (idx, idx))
distances = -2 * distances
distances = distances + XX
distances = distances + YY
ground_metric = torch.sqrt(distances)
    ...
    for i, (image, label) in enumerate(train_loader):
        ...
        if image.shape[2] == 100:
            ...
            for _, prediction in enumerate(outputs):
                ...
                loss = loss + w_loss(prediction, label, ground_metric, args.reg)
